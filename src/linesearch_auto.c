...
static integer c__1 = 1;
 ...
 int lnsrlb_auto(integer *n, double *l, double *u, 
	...
	csave_len) */
{
    ...
    *stpmx = 1e10;
    if (*cnstnd) {
        if (*iter == 0) {
			double* const uR = (double*)mxMalloc(*n*sizeof(double));
			double* const lR = (double*)mxMalloc(*n*sizeof(double));
			double* const uS = (double*)mxMalloc(*n*sizeof(double));
			double* const lS = (double*)mxMalloc(*n*sizeof(double));
			double minS = INFINITY;
			double const sub[] = {-1.0};
			double const add[] = {1.0};
			integer const inc[] = {1};
			integer const dec[] = {-1};
			...
			//	lR[k] = x[k+1]-l[k+1];
			//	uR[k] = u[k+1]-x[k+1];
			//	mexPrintf("[%d] u = %e, l = %e, x = %e, g = %e, nbd = %d\n",k,u[k+1],l[k+1],x[k+1],g[k+1],nbd[k+1]);
			//}
			dcopy(n,x+1,inc,lR,inc);
			dcopy(n,u+1,inc,uR,inc);
			daxpy(n,sub,l+1,inc,lR,inc); // x-l
			daxpy(n,sub,x+1,inc,uR,inc); // u-x
			...
			for(integer k = 0;k < *n;k++)
				lS[k] = max(-d__[k+1],0);
			for(integer k = 0;k < *n;k++) {
				//mexPrintf("uS[%d] = %e\tuR[%d] = %e\n",k,uS[k],k,uR[k]);
				uS[k] = uR[k] / uS[k];
			}
			for(integer k = 0;k < *n;k++) {
				//mexPrintf("lS[%d] = %e\tlR[%d] = %e",k,lS[k],k,lR[k]);
				lS[k] = lR[k] / lS[k];
				//mexPrintf("\tlR/lS = %e\n",lS[k]);
			}
			for(integer k = 0;k < *n;k++)
			...
			for(integer k = 0;k < *n;k++) {
				...
				//mexPrintf("[%d] minS = %e\tlS = %e\tuS = %e\n",k,minS,lS[k],uS[k]);
				minS = min(minS,min(lS[k],uS[k]));
			}
			if(isinf(minS))
				*stpmx = 3e4;
			else
    ...
    if (*iter == 0 && ! (*boxed)) {
        ...
        //*stp = min(d__1,*stpmx);
        *stp = *stpmx / 100;
	mexPrintf("[LBFGSB] Initial step: %1.3e\n",*stp);
    } else {
        *stp = 1.;
    }
