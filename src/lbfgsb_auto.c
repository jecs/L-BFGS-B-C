...
/* ============================================================================= */
/* Subroutine */ int setulb_auto(integer *n, integer *m, double *x, 
...
L111:
    ...
    if (*iprint >= 1) {
        mexPrintf("At iterate %5ld, f(x)= %5.2e, ||proj grad||_infty = %.2e\n",iter,*f,sbgnrm );
    }
    if (sbgnrm <= *pgtol) {
...
L333:
    ...
    if (info != 0) {
        ...
        if (*iprint >= 1) {
            mexPrintf(" Nonpositive definiteness in Cholesky factorization in formk;\n");
            mexPrintf("   refresh the lbfgs memory and restart the iteration.\n");
        }
        info = 0;
        col = 0;
        head = 1;
...
L444:
    if (info != 0) {
        ...
        /*          refresh the lbfgs memory and restart the iteration. */
        if (*iprint >= 1) {
            mexPrintf(" Singular triangular system detected;\n");
            mexPrintf("   refresh the lbfgs memory and restart the iteration.\n");
        }
        info = 0;
        col = 0;
...
L555:
    ...
    timer(&cpu1);
L666:
    lnsrlb_auto(n, &l[1], &u[1], &nbd[1], &x[1], f, &fold, &gd, &gdold, &g[1], &
        ...
        *f = fold;
        if (col == 0) {
            ...
            *task = ABNORMAL;
            ...
            /*             refresh the lbfgs memory and restart the iteration. */
            if (*iprint >= 1) {
                mexPrintf(" Bad direction in the line search;\n");
                mexPrintf("   refresh the lbfgs memory and restart the iteration.\n");
            }
            if (info == 0) {
                --nfgv;
...
L777:
    ...
    if (dr <= epsmch * ddum) {
        ...
        updatd = FALSE_;
        if (*iprint >= 1) {
            mexPrintf("  ys=%10.3e  -gs=%10.3e BFGS update SKIPPED\n", dr, ddum );
        }
        goto L888;
    }
    /* ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc */
    ...
    if (info != 0) {
        ...
        /*          refresh the lbfgs memory and restart the iteration. */
        if (*iprint >= 1) {
            mexPrintf(" Nonpositive definiteness in Cholesky factorization in formt;\n");
            mexPrintf("   refresh the lbfgs memory and restart the iteration.\n");
        }
        info = 0;
        col = 0;
